...
const InactiveDashboard = () => {
  ...
  const nextStep = async () => {
    ...
    try {
      ...
      if (currentStep < totalSteps) {
        ...
        if (currentStep < 3) form.resetFields();
      } else {
        // ✅ Final step -> Submit to API
        const payload = {
          firstName: values.firstName || formData.firstName,
          lastName: values.lastName || formData.lastName,
          dob: (values.dob || formData.dob)?.format("YYYY-MM-DD"),
          country: selected, // ✅ added country
          address: values.address || formData.address, // ✅ added address
          password: values.password || formData.password, // ✅ added password
        ...
        try {
          ...
          console.log(res);
          messageApi.success(res?.data?.message || "Registration successful!");
  ...
  const handlePinSubmit = async (values) => {
    ...
    const payload = { pin: values.pin, email: savedEmail };

    console.log(values);

    console.log("Submitting PIN payload:", payload);
    ...
    try {
      ...
      if (res.data.success) {
        ...
        message.error(res.data.message || "Failed to create PIN");
      }
    } catch (err) {
    ...
    <Layout>
      ...
      <Layout>
        ...
        >
                      ...
                      >
                        <div className="relative">
                          <Input
                            ...
                            value={form.getFieldValue("country") || search}
                            onChange={(e) => {
                          ...
                          {showDropdown &&
                            ...
                            filteredCountries.length > 0 && (
                              <div className="absolute z-50 w-full bg-white mt-2 rounded-md max-h-60 overflow-y-auto">
                                {filteredCountries.map((country) => (
                                  <div
                                    ...
                                    onClick={() => {
                                      ...
                                      setSearch(country.name); // update search display
                  ...
                  <div className="flex justify-center mt-8">
                    ...
                    >
                      {loading
                        ...
                        ? "Submit"
                        : "Next"}
                      {!loading && <HiArrowLongRight size={20} />}
                    </Button>
                  </div>
                </Form>
              </div>
          ...
          <Modal open={pinModal} footer={null} closable={false}>
            <Form
              form={form} // ✅ connect form instance
              layout="vertical"
              onFinish={handlePinSubmit} // ✅ use AntD's form submit
              autoComplete="off"
            >
              <img src={logo} alt="" className="w-40" />
              <h1 className="font-bold text-3xl my-7 mx-18">
                Create Login Pin
              </h1>
              ...
              <div className="flex flex-col gap-4 justify-center items-center">
                ...
                >
                  <Input.OTP
                    length={6}
                    onChange={(val) => form.setFieldValue("pin", val)} // ✅ keep synced
                  />
                </Form.Item>
                ...
                >
                  <Input.OTP
                    length={6}
                    onChange={(val) => form.setFieldValue("confirmPin", val)} // ✅ keep synced
                    disabled={loading}
                  />
                </Form.Item>
              </div>
              ...
              <div className="flex justify-center mt-8">
                <Button
                  type="primary"
                  htmlType="submit" // ✅ triggers form submit
                  loading={loading}
                  className="!bg-[#0047FF] !px-12 !py-4 rounded-lg flex items-center gap-2 !mt-3"
                >
